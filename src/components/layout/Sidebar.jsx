...
export default function Sidebar({ open, collapsed, onClose, onToggleCollapse }) {
  ...
  return <>
    {open && <button type="button" aria-label="Close navigation" onClick={onClose} className="fixed inset-0 z-40 bg-slate-950/60 lg:hidden" />}
    <aside className={[
      'fixed inset-y-0 left-0 z-50 flex flex-col bg-slate-950 text-white shadow-2xl transition-all duration-300 ease-in-out',
      collapsed ? 'lg:w-24' : 'lg:w-72',
      open ? 'w-72 translate-x-0' : 'w-72 -translate-x-full lg:translate-x-0',
    ].join(' ')}>
      <div className="border-b border-white/10 px-4 py-4">
        <div className="flex items-start justify-between gap-2">
          <NavLink to="/dashboard" onClick={handleMobileNavigation} className={collapsed ? 'flex w-full justify-center' : 'flex min-w-0 items-center gap-3'}>
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-blue-600 font-black text-white">CR</div>
            {!collapsed && <div className="min-w-0"><p className="truncate text-lg font-black text-white">CloudRouter</p><p className="truncate text-xs text-slate-400">Hotspot Management Platform</p></div>}
          </NavLink>
          <button type="button" onClick={onClose} className="rounded-xl p-2 text-slate-300 hover:bg-white/10 hover:text-white lg:hidden" aria-label="Close sidebar"><FiX className="h-5 w-5" /></button>
        </div>
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-5">
        {visibleSections.map((section) => <div key={section.title} className="mb-6">
          ...
          ].join(' ')}><Icon className="h-5 w-5 shrink-0" />{!collapsed && <span className="truncate">{item.label}</span>}</NavLink>; })}</div>
        </div>)}
      </nav>

      <div className="border-t border-white/10 p-3">
        {!collapsed && <div className="mb-3 px-3 text-center text-[10px] text-slate-500"><p>{tenantName}</p><p>CloudRouter © {new Date().getFullYear()}</p></div>}
        <button type="button" onClick={onToggleCollapse} className={['hidden w-full items-center rounded-xl bg-white/5 px-3 py-3 text-sm font-medium text-slate-300 hover:bg-white/10 hover:text-white lg:flex', collapsed ? 'justify-center' : 'justify-start gap-3'].join(' ')} aria-label={collapsed ? 'Expand sidebar' : 'Collapse sidebar'} title={collapsed ? 'Expand sidebar' : undefined}>
          {collapsed ? <FiChevronRight className="h-5 w-5"/> : <><FiChevronLeft className="h-5 w-5"/><span>Collapse sidebar</span></>}
        </button>
      </div>
    </aside>
  </>;
